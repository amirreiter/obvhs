...
impl CwBvhNode {
    ...
    pub fn intersect_ray_simd(&self, ray: &Ray, oct_inv4: u32) -> u32 {
        ...
        unsafe {
            // Broadcasts
            let adj_ray_dir_inv_x = vdupq_n_f32(adj_ray_dir_inv.x);
            let adj_ray_dir_inv_y = vdupq_n_f32(adj_ray_dir_inv.y);
            let adj_ray_dir_inv_z = vdupq_n_f32(adj_ray_dir_inv.z);

            let adj_ray_orig_x = vdupq_n_f32(adj_ray_origin.x);
            let adj_ray_orig_y = vdupq_n_f32(adj_ray_origin.y);
            let adj_ray_orig_z = vdupq_n_f32(adj_ray_origin.z);
            ...
            // NEON equivalent of your get_q.
            // Notice the order: x86 _mm_set_ps populates lanes in reverse order.
            // By putting them in a sequential array and using vld1q_f32, we perfectly match
            // your x86 lane layout, and LLVM auto-vectorizes this unpacking brilliantly on AArch64.
            #[inline(always)]
            ...
            // NEON lacks a direct `_mm_movemask_ps`. This trick mimics it with near-zero overhead.
            // It extracts the true/false masks, multiplies them by their bit-position weights,
            // and sums them horizontally.
            #[inline(always)]
            unsafe fn movemask_ps(cmp: uint32x4_t) -> u32 {
                // Shift right by 31 to get 1 or 0 in each lane
                let ones = vshrq_n_u32(cmp, 31);
                // Multipliers for lanes 0..3
                const MULTS: [u32; 4] = [1, 2, 4, 8];
                let mults_vec = vld1q_u32(MULTS.as_ptr());
                let masked = vmulq_u32(ones, mults_vec);
            ...
            for i in 0..2 {
                ...
                // vfmaq_f32(addend, a, b) computes: addend + (a * b)
                let tmin_x = vfmaq_f32(adj_ray_orig_x, x_min, adj_ray_dir_inv_x);
                let tmax_x = vfmaq_f32(adj_ray_orig_x, x_max, adj_ray_dir_inv_x);

                let q_lo_y = get_q(&self.child_min_y, i);
                let q_hi_y = get_q(&self.child_max_y, i);
                let y_min = if rdy { q_hi_y } else { q_lo_y };
                let y_max = if rdy { q_lo_y } else { q_hi_y };
                let tmin_y = vfmaq_f32(adj_ray_orig_y, y_min, adj_ray_dir_inv_y);
                let tmax_y = vfmaq_f32(adj_ray_orig_y, y_max, adj_ray_dir_inv_y);

                let q_lo_z = get_q(&self.child_min_z, i);
                let q_hi_z = get_q(&self.child_max_z, i);
                let z_min = if rdz { q_hi_z } else { q_lo_z };
                let z_max = if rdz { q_lo_z } else { q_hi_z };
                let tmin_z = vfmaq_f32(adj_ray_orig_z, z_min, adj_ray_dir_inv_z);
                let tmax_z = vfmaq_f32(adj_ray_orig_z, z_max, adj_ray_dir_inv_z);

                // Compute intersection
                let tmin = vmaxq_f32(tmin_x, vmaxq_f32(tmin_y, tmin_z));
                let tmax = vminq_f32(tmax_x, vminq_f32(tmax_y, tmax_z));
                let tmin = vmaxq_f32(tmin, vdupq_n_f32(EPSILON));
                let tmax = vminq_f32(tmax, vdupq_n_f32(ray.tmax));

                let intersected = vcleq_f32(tmin, tmax); // true is 0xFFFFFFFF, false is 0x0
                let mask = movemask_ps(intersected);
